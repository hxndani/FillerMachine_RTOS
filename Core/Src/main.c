...
#define BTN_PLUS_Pin      GPIO_PIN_4
#define BTN_PLUS_Port     GPIOB
#define BTN_MIN_Pin       GPIO_PIN_5
#define BTN_MIN_Port      GPIOB
#define BTN_SAVE_Pin      GPIO_PIN_6
#define BTN_SAVE_Port     GPIOB
#define BTN_POWER_Pin     GPIO_PIN_13
#define BTN_POWER_Port    GPIOC

// Output
#define SSR_Pin           GPIO_PIN_5
...
volatile uint32_t previewSetTimer = 60000;
volatile uint8_t previewSpeedMotorPwm = 64;
volatile uint32_t currentSetTimer = 60000;
volatile uint8_t currentSpeedMotorPwm = 64;
volatile uint32_t countdownStart = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  osThreadDef(buttonTask, StartTask_Button, osPriorityLow, 0, 128);
  buttonTaskHandle = osThreadCreate(osThread(buttonTask), NULL);

  osThreadDef(controlTask, StartTask_Control, osPriorityNormal, 0, 128);
  controlTaskHandle = osThreadCreate(osThread(controlTask), NULL);

  osThreadDef(displayTask, StartTask_Display, osPriorityLow, 0, 128);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : BTN_Plus_Pin BTN_Min_Pin BTN_Save_Pin */
  GPIO_InitStruct.Pin = BTN_Plus_Pin|BTN_Min_Pin|BTN_Save_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
...
{
  for(;;) {
    if (HAL_GPIO_ReadPin(BTN_POWER_Port, BTN_POWER_Pin) == GPIO_PIN_RESET) {
      isFilling = !isFilling;
      if (isFilling) {
        HAL_GPIO_WritePin(SSR_Port, SSR_Pin, GPIO_PIN_SET);
        __HAL_TIM_SET_COMPARE(&MOTOR_PWM_TIM, MOTOR_PWM_CHANNEL, motorPwmDuty);
      } else {
        HAL_GPIO_WritePin(SSR_Port, SSR_Pin, GPIO_PIN_RESET);
        __HAL_TIM_SET_COMPARE(&MOTOR_PWM_TIM, MOTOR_PWM_CHANNEL, 0);
      }
    ...
    // handle tombol plus, min, save, dan menu belumm
    osDelay(10); // Polling cepat
  }
}
...
void StartTask_Display(void const * argument) {
  for(;;) {
    ...
    if (isFilling) {
      char buf[16];
      sprintf(buf, "Timer: %lu s", fillTimerMs / 1000);
      lcd_print(buf);
    } else {
